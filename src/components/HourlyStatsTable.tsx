...
export const HourlyStatsTable: React.FC<HourlyStatsTableProps> = ({
  ...
  const getHourlyColumnsData = (hoursList: number[]) => {
    return hoursList.map((hour) => {
      const nextHour = (hour + 1) % 24;
      const label = `${hour.toString().padStart(2, '0')}-${nextHour.toString().padStart(2, '0')}`;
      ...
      const sorted = hourEntries.map(e => conversion(e.sgv)).sort((a, b) => a - b);
  ...
  const renderTable = (hoursList: number[], amPmLabel: string) => {
    const columnsData = getHourlyColumnsData(hoursList);

    return (
      <div className="mb-8 overflow-x-auto border border-slate-200 rounded-xl bg-white p-6 shadow-sm font-sans text-left">
        <div className="mb-4">
          <h3 className="text-sm font-extrabold text-slate-800">Hourly Statistics ({amPmLabel})</h3>
          <p className="text-xs text-slate-400 font-semibold mt-0.5">Aggregated metrics grouped by hour slot</p>
        </div>
          ...
          <thead>
            <tr className="border-b border-slate-200 text-slate-400 font-extrabold text-[10px] uppercase tracking-wider">
              ...
              {columnsData.map((col, idx) => (
                <th key={idx} className="py-3 text-center w-16 font-black">{col.label}</th>
              ))}
            </tr>
          </thead>
  ...
  return (
    <div>
      {renderTable(amHours, '00:00 - 12:00')}
      {renderTable(pmHours, '12:00 - 24:00')}
    </div>
  );
};
